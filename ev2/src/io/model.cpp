                        ...
                        std::string base_dir)
{
    ...
    if (!warn.empty())
    {
        Log::warn_core("Asset warning {}", warn);
    }

    if (!ret)
    {
        Log::error_core("Asset {} loading error {}", filename, err);
        return false;
    }
    ...
    {
        Log::trace_core("material[{}].diffuse_texname = {}\n", int(i),
...

    {
        ...
        for (size_t s = 0; s < shapes.size(); s++)
        {
            ...
            if (buffer.size() - buffer_start > 0)
            {
                ...
                Log::trace_core("shape[{}] # of triangles = {}\n", static_cast<int>(s),
                       o.numTriangles);
                ...
                Log::trace_core("shape[{}] material_id {}\n", int(s), int(o.material_id));
    ...
    if (success) {
        ...
        for (auto& m : materials) {
            ...
            if (!m.ambient_texname.empty())
                mat.ambient_map            = load_image((base_dir / m.ambient_texname).generic_string());
            
            if (!m.diffuse_texname.empty())
                mat.diffuse_map            = load_image((base_dir / m.diffuse_texname).generic_string());
            
            if (!m.specular_texname.empty())
                mat.specular_map           = load_image((base_dir / m.specular_texname).generic_string());
            
            if (!m.specular_highlight_texname.empty())
                mat.specular_highlight_map = load_image((base_dir / m.specular_highlight_texname).generic_string());
            
            if (!m.bump_texname.empty())
                mat.bump_map               = load_image((base_dir / m.bump_texname).generic_string());
            
            if (!m.displacement_texname.empty())
                mat.displacement_map       = load_image((base_dir / m.displacement_texname).generic_string());
            
            if (!m.alpha_texname.empty())
                mat.alpha_map              = load_image((base_dir / m.alpha_texname).generic_string());
            
            if (!m.reflection_texname.empty())
                mat.reflection_map         = load_image((base_dir / m.reflection_texname).generic_string());
